      ...
      last_ambig_(0), revcom_version_(revcom_version)
{
  ...
  if (dna_) {
    set_lookup_table_character('A', 0x1);
    set_lookup_table_character('C', 0x0);
    set_lookup_table_character('G', 0x1);
    set_lookup_table_character('T', 0x0);
  }
  else {
...
uint64_t MinimizerScanner::reverse_complement(uint64_t kmer, uint8_t n) {
  // Create a bitset from kmer; only focus on the lowest n bits
  std::bitset<64> bits(kmer);  // Still using 64-bit for full range
  std::string binary = bits.to_string().substr(64-n);  // Extract the rightmost n bits
  //
  // // Reverse the extracted binary string
  std::reverse(binary.begin(), binary.end());
  //
  // // Convert the reversed binary string back to a uint64_t
  std::bitset<64> reversed_bits(binary);  // Fill from the right
  //
  // // Step 4: Bitwise complement of the reversed bits (flipping bits)
  reversed_bits = ~reversed_bits;
  //
  // // Mask off bits to ensure only n bits are complemented (important for lengths not equal to 64)
  std::bitset<64> mask((1ULL << n) - 1);  // Creates a mask with n bits set to 1
  reversed_bits &= mask;
  //
  if (revcom_version_ == 0)
    // This branch present to maintain backwards compatibility with old DBs
    return (~kmer) & ((1ull << (n * 1)) - 1);  
  else
    return reversed_bits.to_ullong();  // Convert bitset to uint64_t
