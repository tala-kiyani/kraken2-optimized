    ...
    CompactHashTable &hash, const Taxonomy &tax)
{
  ...
  //std::ofstream outfile("/home/tala.kiani78.sharif/.conda/envs/krakenme/kraken2-2.1.2/seq.txt");
  //if (!outfile)
  //{
    //std::cerr << "Error opening file: seq.txt" << std::endl;
    //return;
  //}

  // Write seq with line numbers to the file
  //size_t lineNum = 1;
  //std::istringstream iss(seq);
  //std::string line;
  //while (std::getline(iss, line))
  //{
    //outfile << "Sequence #" << lineNum << ": " << line << std::endl;
    //lineNum++;
  //}

  // Close the output file
  //outfile.close();
  //std::cout << "Sequence written to file: seq.txt" << std::endl;
  ...
  for (size_t j = 0; j < seq.size(); j += opts.block_size) {
    ...
    for (size_t i = block_start; i < block_finish; i += opts.subblock_size) {
      ...
      while ((minimizer_ptr = scanner.NextMinimizer())) {
        if (scanner.is_ambiguous())
          continue;
         // Write minimizer_ptr to a text file
        //std::ofstream outputFile("/home/tala.kiani78.sharif/.conda/envs/krakenme/kraken2-2.1.2/minimizers.txt", std::ios::app);
        //if (outputFile.is_open()) {
          //outputFile << *minimizer_ptr << "\n";
          //outputFile.close();
//        } else {
          // Handle error opening the file
  //        std::cerr << "Error opening the file.\n";
          //Optionally, you can choose to exit the program or handle the error differently
    //      exit(1);
      //  }
  ...
  while ((opt = getopt(argc, argv, "?hB:b:c:FH:m:n:o:t:k:l:M:p:r:s:S:T:X")) != -1) {
    switch (opt) {
      ...
      case 'l' :
        ...
        if (sig > 50)
          errx(EX_USAGE, "l must be no more than 50");
        opts.l = sig;
        break;
      case 'c' :
        sig = atoll(optarg);
